...
import { canCompleteTrip, canStartTrip, getAuthoritativeFinalFare, getQuotedRideFare, getTripChargeTotal, roundGhsFare } from '../src/fareAuthority';
...
describe('HY3N fare authority', () => {
  ...
  it('never allows a fare to be completed before Start Trip', () => {
    expect(canStartTrip({ status: 'driver_arriving' })).toBe(false);
    expect(canStartTrip({ status: 'driver_arrived' })).toBe(true);
    expect(canCompleteTrip({ status: 'driver_arrived', driver_arrived_at: '2026-09-24T08:00:00.000Z' })).toBe(false);
    expect(canCompleteTrip({ status: 'in_progress' })).toBe(false);
    expect(canCompleteTrip({ status: 'in_progress', trip_started_at: '2026-09-24T08:01:00.000Z' })).toBe(true);
  });
});
